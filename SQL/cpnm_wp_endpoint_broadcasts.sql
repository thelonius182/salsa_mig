...
SELECT
    ...
    JSON_OBJECT(
        'nl', MAX(CASE WHEN locale = 'nl' THEN title END),
        'en', MAX(CASE WHEN locale = 'en' THEN title END)
    ) AS title,

    JSON_OBJECT(
        'nl', MAX(CASE WHEN locale = 'nl' THEN description END),
        'en', MAX(CASE WHEN locale = 'en' THEN description END)
    ) AS description,

    MAX(genre_keys) AS genre_keys,

    MAX(maker) AS maker,

    JSON_OBJECT(
        'nl', MAX(CASE WHEN locale = 'nl' THEN maker_role END),
        'en', MAX(CASE WHEN locale = 'en' THEN maker_role END)
    ) AS maker_role,

    MAX(stream_url) AS stream_url,
    MAX(mood_raw) AS mood_raw,

    MAX(CASE WHEN locale = 'nl' THEN post_id END) AS wp_post_id_nl,
    MAX(CASE WHEN locale = 'en' THEN post_id END) AS wp_post_id_en
